  ...
  const handleRetryProcessing = async () => {
    if (selectedFile) {
      try {
        ...
        const result = await retryProcessing(selectedFile, ocrSettings);
        ...
        if (result) {
          ...
          console.log('Calling onDataExtracted with:', result);
          onDataExtracted?.(result);
        }
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Error al reintentar procesamiento';
